...
export default function StudentDashboard({
  ...
  const [viewLeaderboardQuizId, setViewLeaderboardQuizId] = useState<
    string | null
  >(null);
  ...
  const myQuizzes = quizzes.filter(
    (q) => myClasses.some((c) => c.id === q.classId) && q.visible
  );
  ...
  const getAverageScore = () => {
    const myAttempts = attempts.filter(
      (att) => att.studentId === currentUser.id
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto p-4 space-y-4">
        ...
        <Tabs defaultValue="quizzes" className="space-y-4">
          ...
          <TabsContent value="quizzes" className="space-y-4">
            {myClasses.map((cls) => {
              const classQuizzes = myQuizzes.filter(
                (q) => q.classId === cls.id
              );
                          ...
                          >
                            ...
                            <div className="flex gap-2">
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  setViewLeaderboardQuizId(quiz.id)
                                }
                              >
                                Leaderboard
                              </Button>
      ...
      <Dialog
        open={!!viewLeaderboardQuizId}
        onOpenChange={() => setViewLeaderboardQuizId(null)}
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              Leaderboard -{" "}
              {quizzes.find((q) => q.id === viewLeaderboardQuizId)?.name}
            </DialogTitle>
          </DialogHeader>
